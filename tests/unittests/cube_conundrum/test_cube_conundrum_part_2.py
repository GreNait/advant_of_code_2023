from src.cube_conundrum.cube_conundrum import CubeConundrumExtended
...
test_power_of_cubes = [48, 12, 1560, 630, 36]
...
def test_get_minimum_cubes_of_game():
    ...
    cube_conundrum = (
        CubeConundrumExtended()
    )  # games=load_games.games, limits=test_limits
    for game, minimum_cubes in zip(test_batch, test_minimum_cubes):
        assert sorted(
            cube_conundrum._min_cubes_of_game(game=test_batch[game])
        ) == sorted(minimum_cubes)
