...
class GearRatios:
    def __init__(self, lines: str = None):
        if lines:
            ...
            self._adjacent_numbers = [
                number for number in self._all_numbers if number[1] is True
            ]
    ...
    def _find_symbols_by_numbers(self, numbers: list, lines: str) -> list[tuple]:
        ...
        for number in numbers:
            number_symbol.append(
                (
                    int(number.number),
                    self._find_adjacent_symbol(lines=lines, number=number),
                )
            )
    ...
    def _find_adjacent_symbol(self, lines: str, number):
        ...
        symbols = self.__get_all_symbols(
            lines
        )  # TODO: search only for symbols in range
